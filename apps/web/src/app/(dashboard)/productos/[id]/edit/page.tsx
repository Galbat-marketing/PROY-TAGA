...
import { actualizarProducto, getProducto, subirImagenProducto } from "@/lib/actions/productos"
...
export default function EditarProductoPage() {
  ...
  useEffect(() => {
    getProducto(id).then((data) => {
      const prod = data as unknown as Producto
      setProducto(prod)
      reset(data as unknown as ProductoFormValues)
      if (prod.imagen_url) {
  ...
  async function onSubmit(values: ProductoFormValues) {
    try {
      if (imagenFile) {
        const imagenUrl = await subirImagenProducto(id, imagenFile)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
      <div className="rounded-xl border border-border bg-card p-6 space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <select {...register("categoria_id")} className="flex h-11 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm">
      ...
      <div className="flex gap-3">
        <Button type="submit" disabled={isSubmitting}>
          <Save className="h-4 w-4" /> {isSubmitting ? "Guardando..." : "Actualizar producto"}
        </Button>
  ...
  return (
    <div>
      <PageHeader title="Editar Producto" description={producto?.nombre ?? ""}>
